          ...
          leading: Row(
            ...
            children: [
              ...
              IconButton(
                  ...
                  onPressed: ()=> Navigator.pushNamed(context, DashBoardView.route)),
            ],
          ),),
                    ...
                    BlocBuilder <NotificationBloc, NotificationState>(
                      builder: ((context, state) {
                        ...
                        if (state.appState== AppState.failed){
                          return Center(child: Text('', style: FastaTextStyle.subtitle1,),);
                        } else
                        if (state.appState== AppState.success){
                          if (state.entityList.isEmpty){return Text('No Notifications');}
                          else {return Column(
                          children: [
                            Row(mainAxisAlignment: MainAxisAlignment.start,
                              children: [
                                Text(state.unreadCount.toString(), style: FastaTextStyle.subtitle1,),
                              ],
                            ),
                            Column(
                              children: 
                                List.generate(state.entityList.length, (index) {
                                  ...
                                  return GestureDetector(
                                      onTap: () {
                                        ...
                                        Navigator.pushReplacementNamed(context, NotificationMessageView.route, 
                                        arguments: state.entityList[index].message.toString());
                                        context.read<NotificationBloc>().add(MarkUserReadEvent(state.entityList[index].id));
                                      },
                                      child: FeatureBuilder(
                                          name: state.entityList[index].title, icon: state.entityList[index].isRead? Icons.mark_as_unread_outlined:Icons.mark_as_unread),
                                    );
