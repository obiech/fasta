        ...
        builder: (context, state) {
          return Column(
            children: [
              ...
              Text('Please enter the 6 digit code sent to ${state.user?.email}',
              ...
              OtpTextField(
                ...
                filled: true,
                onSubmit: (_) {
                  context.read<ProfileBloc>().add(ProfileEvent.verifyEmail(
                      VerifyEmailArg(_, state.otpId ?? '',state.user?.userId.toString()??'')));
                },
              ),
              SizedBox(
                height: 25.h,
              ...
              GestureDetector(
                onTap: () {
                  context.read<ProfileBloc>().add(
                      ProfileEvent.resendEmailOtp(state.user?.email?? ''));
                },
                child: Text(
                  'Resend code',
